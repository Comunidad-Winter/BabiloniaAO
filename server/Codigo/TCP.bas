...
With UserList(UserIndex)
    ...
    ElseIf UserClase = eClass.Arquero Then ' Mana inicial de Tu clase
            .Stats.MaxMAN = 45
            .Stats.MinMAN = 45
    Else
        .Stats.MaxMAN = 0
...
On Error GoTo Errhandler
    ...
    If UserList(UserIndex).ConnID <> -1 Then
        Call CloseSocketSL(UserIndex)
    End If
     If UserList(UserIndex).nDuel.ForTheWorld Then
       
        If Duelos_UsuariosEnDuelo(UserIndex, UserList(UserIndex).nDuel.OtherIndex) Then
           Duelos_TerminarDuelo UserList(UserIndex).nDuel.OtherIndex, UserIndex, False, True
        End If
...
With UserList(UserIndex)
    ...
    Call WriteChangeMap(UserIndex, .Pos.Map, MapInfo(.Pos.Map).MapVersion, MapInfo(.Pos.Map).name)
