...
Dim DruidManaBonus As Single
    ...
    With UserList(UserIndex)
        ...
        If .Stats.MinMAN < Hechizos(HechizoIndex).ManaRequerido * DruidManaBonus Then
            ...
            Call SendData(SendTarget.ToPCArea, UserIndex, PrepareMessagePlayWave(NOMANA, .Pos.X, .Pos.Y))
            Exit Function
        End If
...
With UserList(targetIndex)
    ...
    ElseIf Hechizos(SpellIndex).SubeHP = 2 Then
        ...
        If daño < 150 Then
            Call WriteChatOverHead(UserIndex, daño, UserList(UserIndex).Char.CharIndex, vbRed)
            Call WriteChatOverHead(targetIndex, daño, UserList(UserIndex).Char.CharIndex, vbRed)
        Else
            Call WriteChatOverHead(UserIndex, "¡" & daño & "!", UserList(UserIndex).Char.CharIndex, vbYellow)
            Call WriteChatOverHead(targetIndex, "¡" & daño & "!", UserList(UserIndex).Char.CharIndex, vbYellow)
        End If
