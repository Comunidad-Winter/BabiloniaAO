...
Private Enum ClientPacketID
    ...
    ActivarDeath            '/ACDEATH CUPOS@CAENOBJS
    IngresarDeath           '/DEATH
...
On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.Meditate                '/MEDITAR
            ...
            Case ClientPacketID.Sala1                   '/SALA1
            Call HandleSala1(UserIndex)
            
            Case ClientPacketID.nDuelos
            Call HandlenDuelos(UserIndex)
...
#End If
        ...
        If BANCheck(UserName) Then
           'Razon por el ban
        Call WriteErrorMsg(UserIndex, "Se te ha prohibido la entrada a Argentum Online debido a " & ban_Reason(UserName))
...
Call WriteConsoleMsg(UserIndex, "Comando solo para Game Masters", FontTypeNames.FONTTYPE_INFOBOLD)
 Exit Sub
 End If
Dim gema As Obj
gema.Amount = 1
...
'acá aparece en el usuario
 Call WriteConsoleMsg(tUser, "El administrador " & .name & " te ha entregado " & gema.Amount & " Gema Lunar.", FontTypeNames.FONTTYPE_CENTINELA)
Call .incomingData.CopyBuffer(buffer)
End With
Errhandler:
 Dim error As Long
...
Err.Raise error
End Sub
Private Sub HandlenDuelos(ByVal UserIndex As Integer)
 
' \  Note   :  Paquete para iniciar duelo con targetUser
' \  Author :  maTih.-
...
tIndex = .flags.TargetUser
 
'No seleccionó un usuario.
 
If tIndex <= 0 Then
   WriteConsoleMsg UserIndex, "Debes seleccionar un usuario.", FontTypeNames.FONTTYPE_CITIZEN
   Exit Sub
End If
 
'Pasa las comprobaciones?
 
If Duelos_IndexsPueden(UserIndex, tIndex, RefString) Then
 
    'Seteamos el flag y enviamos.
    .nDuel.OtherIndex = tIndex
    Duelos_IniciarDuelo UserIndex, tIndex
Else
   
    'Larga vida a vb y ByRef!
    WriteConsoleMsg UserIndex, RefString, FontTypeNames.FONTTYPE_CITIZEN
End If
...
'***************************************************
    With UserList(UserIndex)
        ...
        If Not Ayuda.Existe(.name) Then
            ...
            Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(LCase$(.name) & " SOPORTE>: " & "Un Usuario a tipeado /GM Valla a atenderlo!!!!" & Text, FontTypeNames.FONTTYPE_GUILDMSG))
        Else
        ...
        Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(LCase$(.name) & " SOPORTE>: " & "Anda a atender al Usuario culiado" & Text, FontTypeNames.FONTTYPE_GUILDMSG))
        End If
    End With
End Sub
...
Call Statistics.ParseChat(chat)
If HayGlobal Then _
Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg(.name & "> " & chat, FontTypeNames.FONTTYPE_VENENO))
If HayGlobal = False Then
Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El Global No está Activado. Intentelo más tarde.", FontTypeNames.FONTTYPE_VENENO))
End If
...
Private Sub HandleGlobalOn(ByVal UserIndex As Integer)
...
Private Sub HandleGlobalOff(ByVal UserIndex As Integer)
...
End With
End Sub
Public Sub HandleSala1(ByVal UserIndex As Integer)
With UserList(UserIndex)
Call .incomingData.ReadByte
 
    If .flags.Muerto = 1 Then
       Call WriteConsoleMsg(UserIndex, "Los muertos no pueden entrar a la sala", FONTTYPE_CONSEJOCAOS)
    Else
       Call WarpUserChar(UserIndex, 293, 32, 81, True) 'Aca modifican el 100 por el mapa qe qieren que sea, y el 50,50 , por las coordenadas que quieren que sea.
       Call SendData(SendTarget.ToAll, UserIndex, PrepareMessageConsoleMsg(.name & " Ha entrado a la SALA numero 1", FontTypeNames.FONTTYPE_INFO))
       Call WriteConsoleMsg(UserIndex, "Has sido teletranspotado a la SALA numero 1", FONTTYPE_CENTINELA)
          End If
              End With
                End Sub
