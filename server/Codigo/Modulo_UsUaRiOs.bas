...
On Error GoTo Errhandler
    ...
    With UserList(UserIndex)
        Do While .Stats.Exp >= .Stats.ELU
            ...
            Call WriteChatOverHead(UserIndex, " " & " Vida+" & AumentoHP & " Mana+" & AumentoMANA & " Golpe+" & AumentoHIT & ".", str(UserList(UserIndex).Char.CharIndex), vbCyan)
...
On Error GoTo ErrorHandler
    ...
    With UserList(UserIndex)
    If .Invent.AnilloEqpObjIndex = RYKAN Then
    .Stats.MinHp = .Stats.MaxHp
If .flags.Muerto = 1 Then .flags.Muerto = 0
Call WriteConsoleMsg(UserIndex, "No has muerto en la batalla, ni has perdido tus objetos gracias al Collar Mitologico.", FontTypeNames.FONTTYPE_INFO)
...
End If
        ...
        If criminal(UserIndex) Then
Call WriteChatOverHead(UserIndex, "¡¡¡Ahhhhhhhhhhhhhhhhhhh!!!", UserList(UserIndex).Char.CharIndex, vbRed)
Else
Call WriteChatOverHead(UserIndex, "¡¡¡Ahhhhhhhhhhhhhhhhhhh!!!", UserList(UserIndex).Char.CharIndex, vbBlue)
End If
    ...
    End If
        ...
        If .nDuel.ForTheWorld = False Then
        ...
        'Está dueleando?
        If .nDuel.ForTheWorld Then
       
        Duelos_TerminarDuelo .nDuel.OtherIndex, UserIndex
        ...
        If OldMap <> Map Then
           Call WriteChangeMap(UserIndex, Map, MapInfo(.Pos.Map).MapVersion, MapInfo(Map).name)
            ...
            If .nDuel.ForTheWorld Then
                ...
                If Duelos_UsuariosEnDuelo(UserIndex, .nDuel.OtherIndex) Then
                    Duelos_TerminarDuelo .nDuel.OtherIndex, UserIndex, True
                End If
