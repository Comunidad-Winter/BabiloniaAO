...
On Error Resume Next
    ...
    Open IniPath & "MiniMap.dat" For Binary As handle
        Seek handle, 1
          ...
          If .invisible And CharIndex = UserCharIndex Then
                ...
                If .Head.Head(.Heading).GrhIndex Then
                    ...
                    If .Casco.Head(.Heading).GrhIndex Then _
                        Call DDrawTransGrhtoSurfaceAlpha(.Casco.Head(.Heading), PixelOffsetX + .Body.HeadOffset.X, PixelOffsetY + .Body.HeadOffset.Y + OFFSET_HEAD, 1, 0)
...
rr = RandomNumber(1, 255) 'Colores by sxex s-a
gg = RandomNumber(1, 255) 'Colores by sxex s-a
bb = RandomNumber(1, 255) 'Colores by sxex s-a
color = RGB(rr, gg, bb)
End If
                            'Nick
                            line = Left$(.Nombre, Pos - 2)
                            Call RenderTextCentered(PixelOffsetX + TilePixelWidth \ 2 + 5, PixelOffsetY + 30, line, color, frmMain.lblfontnick.font)
