...
'For now we just drop it
    ...
    frmMain.lblmapaname.Caption = incomingData.ReadASCIIString()
...
'
' @remarks  The data is not actually sent until the buffer is properly flushed.
Public Sub WritenDuelos()
 
' \  Note   :  Envia el paquete para crear un duelo
' \  Author :  maTih.-
 
Call outgoingData.WriteByte(ClientPacketID.nduelos)
...
'
' @param    cuenta the countdown's start value
' @param    map the map where the countdown will be sent (0 equals ALL maps = ALL users).
' @remarks  The data is not actually sent until the buffer is properly flushed.
 
Public Sub WriteCountdown(ByVal Cuenta As Integer, ByVal Map As Integer)
'***************************************************
'Author: Nicolas Javier Amoruso (NaKruL)
'Last Modification: 03/06/10
'Writes the "Countdown" message to the outgoing data buffer
'***************************************************
    With outgoingData
            Call .WriteByte(ClientPacketID.GMCommands)
...
End With
End Sub
Public Sub writeSala1()
With outgoingData
  Call .WriteByte(ClientPacketID.Sala1)
   End With
End Sub
